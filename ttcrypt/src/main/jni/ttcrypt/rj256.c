...
/* Rijndael Cipher
   ...
   Version adopted for Java/Ruby by real.sergeych@gmail.com
*/
...
static void rj256_rijndael_gentables(void)
{				/* generate tables */
	int i;
	byte y, b[4];
...
int rj256_set_key(RI * rinst, byte * key)
{
	int nk = 32;
...
void rj256_encrypt(RI * rinst, byte * buff)
{
	int i, j, k, m;
...
void rj256_decrypt(RI * rinst, byte * buff)
{
	int i, j, k, m;
	word32 a[8], b[8], *x, *y, *t;
...
int rj256_encrypt_cbc( byte* key, byte* _iv, uint* src, int length, uint* dst ) {
	uint iv[8];
	uint *piv;
	RI ri;
	
	rj256_set_key( &ri, key );

	memcpy( iv, _iv, 32 );
	
	if( length < 32 || (length & 0x1f) != 0 )
		return ERR_SIZE;
...
int rj256_decrypt_cbc( byte* key, uint* piv, uint* src, int length, uint* dst ) {
	RI ri;
	uint buff[8];
	uint *pbuff;
...
//int rj256_get_key_size()
//{
//	return 32;
//}
//
//static const int key_sizes[] = { 16, 24, 32 };
//static const int *rj256_get_supported_key_sizes(int *len)
//{
//	*len = sizeof(key_sizes)/sizeof(int);
//	return key_sizes;
//
//}
//char *rj256_get_algorithms_name()
//{
//return "Rijndael-256";
//}
...
//
//	rj256_set_key(key, (void *) keyword);
//	free(keyword);
//
//	rj256_encrypt(key, (void *) ciphertext);
//
//	for (j = 0; j < blocksize; j++) {
//		_snprintf(&((char *) cipher_tmp)[2 * j],
//			sizeof(cipher_tmp),
//			"%.2x",
//			ciphertext[j]);
//	}
...
//		return -2;
//	}
//	rj256_decrypt(key, (void *) ciphertext);
//	free(key);
//
